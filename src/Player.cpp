...
Player::Move Player::move(Board& board, bool tournament,
...
void Player::printMove(Move move, int flips, bool tournament) const {
  if (move) {
    assert(flips > 0);
    if (!tournament)
      std::cout << color << " played at: " << *move << " (" << flips << " flip"
                << (flips > 1 ? "s" : "") << ")\n";
  }
}
...
Player::Move HumanPlayer::makeMove(Board& board, const Board::Moves&,
                                   int& flips) const {
  do {
...
#ifdef __clang__
  // g++ uses libstdc++ (instead of libc++) which only understands locale "C".
  // Other values cause a runtime exception: locale::facet::_S_create_c_locale
...
Player::Move ComputerPlayer::makeMove(Board& board, const Board::Moves&,
                                      int& flips) const {
...
int ComputerPlayer::minMax(const Board& board, int depth, Board::Color turn,
  ...
  // if no valid moves for current player then go to next level unless there
  // were no valid moves for previous level - in this case stop traversing and
  // return score (by setting depth to 0)
  if (moves == 0)
    return callMinMax(board, prevMoves ? nextLevel : 0, Board::opColor(turn), 0,
                      alpha, beta);
  // maximizing player
  if (turn == color) {
    int best = Min;
...
RemotePlayer::RemotePlayer(Board::Color c, bool debug)
    : Player(c), _debug(debug),
      _acceptor(_service, tcp::endpoint(tcp::v4(), Port)), _socket(_service),
      _input(&_inputBuffer) {}

using namespace boost::asio;

void RemotePlayer::waitForConnection(const Board& board,
                                     const Board::Moves& prevMoves) const {
  ...
  if (auto clientType = get(); clientType == "printBoards") {
    _print = true;
    send(prevMoves);
    // always send initial board string even if previous move is empty, i.e.: we
    // are first player
    send(board.toString());
  } else
    send(prevMoves);
...
void RemotePlayer::gameOver(const Board& board,
                            const Board::Moves& prevMoves) const {
  send("end");
  send(prevMoves);
...
Player::Move RemotePlayer::makeMove(Board& board, const Board::Moves& prevMoves,
                                    int& flips) const {
  if (!_socket.is_open())
