...
void Game::begin() {
  ...
  do {
    if (_matches) {
      const auto width = _matches < 10     ? 2
                         : _matches < 100  ? 3
                         : _matches < 1000 ? 4
                                           : 5;
  ...
  if (_matches > 1)
    std::cout << ">>> Black Wins: " << blackWins
              << ", White Wins: " << whiteWins << ", Draws: " << draws
...
Board Game::playOneGame() {
  ...
  // inform the other player that the game is over and pass final move(s) made
  // by last player
...
char Game::getChar(Board::Color c, const std::string& msg,
                   const std::string& choices, bool pred(char), char def) {
  std::string line;
  do {
    std::cout << ">>> " << c << " - " << msg << " (" << choices << ") default '"
              << def << "': ";
...
std::unique_ptr<Player> Game::createPlayer(Board::Color c) {
  ...
  static const auto typeRemotePred = [](char x) {
    return typePred(x) || x == 'r';
  };
  // if a tournament option is chosen then both players will be 'c' (Computer)
  // type, otherwise the board is printed each turn (currently only one player
  // can be 'remote')
  auto type =
    _matches           ? 'c'
    ...
    : _players.empty()
      ? getChar(
          ...
          [](char x) { return typeRemotePred(x) || (x >= 'w' && x <= 'z'); },
          'y')
  ...
  const auto search = getChar(
    c, "search depth", "0=no search, 1-9=moves",
    [](char x) { return x >= '0' && x <= '9'; }, '3');
  const auto random = getChar(
    c, "randomized results", "y/n", [](char x) { return x == 'y' || x == 'n'; },
    'y');
  std::shared_ptr<Score> score = nullptr;
  if (search != '0') {
    type = getChar(
      c, "score type", "f=full heuristic, w=weighted cells",
      [](char x) { return x == 'f' || x == 'w'; }, 'f');
    if (type == 'f')
      score = std::make_shared<FullScore>();
    else
      score = std::make_shared<WeightedScore>();
  }
  return std::make_unique<ComputerPlayer>(c, search - '0', random == 'y',
                                          score);
}
